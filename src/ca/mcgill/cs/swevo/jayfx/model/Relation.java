...
public enum Relation {
	...
	EXPLICITLY_CALLS(Type.ID_EXPLICITLY_CALLS, true), CHECKS(Type.ID_CHECKS, true), CREATES(Type.ID_CREATES,
			true), DECLARES(Type.ID_DECLARES, true), EXTENDS_CLASS(Type.ID_EXTENDS_CLASS, true), EXTENDS_INTERFACES(
					Type.ID_EXTENDS_INTERFACES, true), HAS_PARAMETER_TYPES(Type.ID_HAS_PARAMETER_TYPES,
							true), HAS_RETURN_TYPE(Type.ID_HAS_RETURN_TYPE, true), IMPLEMENTS_INTERFACE(
									Type.ID_IMPLEMENTS_INTERFACE,
									true), OF_TYPE(Type.ID_OF_TYPE, true), TRANS_EXTENDS(Type.ID_TRANS_EXTENDS,
											true), TRANS_IMPLEMENTS(Type.ID_TRANS_IMPLEMENTS, true),

	ACCESSES(Type.ID_ACCESSES, true), CALLS(Type.ID_CALLS, true), IMPLEMENTS_METHOD(Type.ID_IMPLEMENTS_METHOD,
			true), INHERITS(Type.ID_INHERITS, true), OVERRIDES(Type.ID_OVERRIDES, true), USES(Type.ID_USES, true),
	...
	public boolean hasDomainCategory(ICategories pCategory) {
		...
		if (pCategory == ICategories.CLASS) {
			if (this == DECLARES || this == EXTENDS_CLASS || this == EXTENDS_INTERFACES || this == IMPLEMENTS_INTERFACE
					|| this == INHERITS || this == REFERENCES || this == T_CHECKS || this == T_CREATES
					|| this == T_EXTENDS_CLASS || this == T_EXTENDS_INTERFACES || this == T_HAS_PARAMETER_TYPES
					|| this == T_HAS_RETURN_TYPE || this == T_IMPLEMENTS_INTERFACE || this == T_OF_TYPE
					|| this == T_USES || this == IDENTITY || this == T_IDENTITY || this == TRANS_EXTENDS
					|| this == TRANS_IMPLEMENTS || this == T_TRANS_EXTENDS || this == T_TRANS_IMPLEMENTS
					|| this == T_REFERENCES)
				lReturn = true;
		} else if (pCategory == ICategories.METHOD) {
			if (this == ACCESSES || this == CALLS || this == EXPLICITLY_CALLS || this == CHECKS || this == CREATES
					|| this == HAS_PARAMETER_TYPES || this == HAS_RETURN_TYPE || this == IMPLEMENTS_METHOD
					|| this == OVERRIDES || this == USES || this == REFERENCES || this == T_EXPLICITLY_CALLS
					|| this == T_DECLARES || this == T_CALLS || this == T_IMPLEMENTS_METHOD || this == T_INHERITS
					|| this == T_OVERRIDES || this == T_USES || this == IDENTITY || this == T_IDENTITY
					|| this == STATIC_CALLS || this == T_STATIC_CALLS || this == T_REFERENCES)
				lReturn = true;
	...
	public boolean isUnion() {
		boolean lReturn = false;
		if (aId == Type.ID_ACCESSES || aId == Type.ID_CALLS || aId == Type.ID_USES || aId == Type.ID_REFERENCES)
			lReturn = true;
		return lReturn;
	}
	...
	public enum Type {
		 ...
		 * Primitive: 0-9
		 */
		ID_EXPLICITLY_CALLS("explicitly-calls", "e-calling", "e-called by",
				"a method calls a method explicitly"), ID_CHECKS("checks", "checking", "checked by",
						"a method checks the run-time type of an object for a specific type"), ID_CREATES("creates",
								"creating", "created by",
								"a method creates an object of a type"), ID_DECLARES("declares", "declaring",
										"declared by", "a class declares a member"), ID_EXTENDS_CLASS("extends-class",
												"extending", "extended by",
												"a class directly extends a class"), ID_EXTENDS_INTERFACES(
														"extends-interface", "i-extending", "i-extended by",
														"an interface directly extends an interface"), ID_HAS_PARAMETER_TYPES(
																...
																"a method has parameters of types"), ID_HAS_RETURN_TYPE(
																		...
																		"a method has return type"), ID_IMPLEMENTS_INTERFACE(
																				"implements-interface", "implementing",
																				"implemented by",
																				"a class directly implements an interface"), ID_OF_TYPE(
																						"of-type", "being of type",
																						"type of",
																						"a field is of type"),

		ID_TRANS_EXTENDS("transitively-extends", "transitively extending", "transitively extended by",
				"a class transitively extends a class"), ID_TRANS_IMPLEMENTS("transitively-implements",
						"transitively implementing", "transitively implemented by",
						"a class transitively implements an interface"),

		ID_ACCESSES("accesses", "accessing", "accessed by",
				"a method accesses a field, either to read or write"), ID_CALLS("calls", "calling", "called by",
						"a method calls a method"), ID_IMPLEMENTS_METHOD("implements-method", "m-implementing",
								"m-implemented by", "a method implements an interface method"), ID_INHERITS("inherits",
										"inheriting", "inherited by",
										"a class inherits fields and methods"), ID_OVERRIDES("overrides", "overriding",
												"overridden by", "a method overrides a method"), ID_USES("uses",
														"using", "used by", "a method uses a program element"),

		ID_IDENTITY("identity", "is", "is", "an element is itself"), ID_STATIC_CALLS("statically-calls",
